...
export async function discoverPlugins(context, presets = []) {
	const dependencies = await findDependenciesFromContext(
		context,
		(name) => name.startsWith('@ovenwand/toolchain.plugins.') || name.includes('toolchain-plugin-')
	);

	const presetDependencies = await Promise.all(
		findDependencies(
			presets,
			(name) =>
				name.startsWith('@ovenwand/toolchain.plugins.') || name.includes('toolchain-plugin-')
		)
	);
