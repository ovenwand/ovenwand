...
export function findDependencies(
	...
	filter: (name: string) => boolean | unknown
): Promise<Dependency>[] {
	const dependencyQueue = [];

	for (const { manifest, path } of manifests) {
		const { devDependencies, dependencies } = manifest;
		const manifestDependencies = Object.keys({ ...devDependencies, ...dependencies }).filter(
			filter
		);

		for (const dependencyName of manifestDependencies) {
			dependencyQueue.push(readDependency(dependencyName, path));
		}
	}
...
export async function findDependenciesFromContext(
	context,
	filter: (name: string) => boolean | unknown
): Promise<Dependency[]> {
	const { meta } = context;
