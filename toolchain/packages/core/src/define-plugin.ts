...
export type DefineToolchainPlugin = (context: Toolchain.Context) => Partial<ToolchainPlugin> | Promise<Partial<ToolchainPlugin>>;
...
export function definePlugin(plugin: ToolchainPluginDefinition | DefineToolchainPlugin): DefineToolchainPlugin {
	return (context) => {
		const pluginDefinition: Partial<ToolchainPlugin> = typeof plugin === 'function' ? plugin(context) : plugin;
