...
export default async (request, response) => {
  ...
  app.webhooks.on("pull_request.opened", async ({ octokit, payload }) => {
    ...
    const { createTestReport } = useTestReport(
      { name: "Test end-to-end", pull_request },
      repository,
      octokit
    );
  ...
  app.webhooks.on("issue_comment.edited", async ({ octokit, payload }) => {
    const { comment, issue } = payload;
    console.log(payload, comment, issue);
  });
