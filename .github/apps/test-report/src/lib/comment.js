...
/**
 * @typedef {import('@octokit/webhooks-types/schema.js').PullRequestOpenedEvent | import('@octokit/webhooks-types/schema.js').WorkflowRun} TestReportEvent
 */

/**
 * @typedef {Object} Report
 * @property {import('./constants.js').TestSuite} suite
 * @property {import('./constants.js').TestStatus} status
 * @property {import('./constants.js').Verdict} verdict
 ...
 * @param {import('octokit').Octokit} octokit
 */
export async function createTestReport(event, repository, octokit) {
  const [pullRequest] = event.pull_request ? [event.pull_request] : event.pull_requests;
...
export async function updateTestReport(suite, event, repository, octokit, mutation) {
  const [pullRequest] = event.pull_requests?.length ? event.pull_requests : [event.pull_request];
