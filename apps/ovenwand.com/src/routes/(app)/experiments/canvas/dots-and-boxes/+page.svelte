<script lang="ts">
  ...
  import { debug, faceToAdjacentFaceMap, faceToDirectionMap, faceToLineOffsetMap } from "./lib/utils";

  const width = 480;
  const height = 480;

  let canvas;
  let grid, points, squares, lines;
  let gridSize, offset;
  ...
  setup(({ onClick, onMouseMove, onKeyDown }) => {
    ...
    onClick((mouseX, mouseY) => {
      ...
      const selectedSquareIndex = squares.findIndex((square) => typeof square.selected === 'string');
      ...
      debug(`Player ${currentPlayer + 1} attempting to draw a line on the ${selected} side of the square at (${position.x},${position.y}).`);
  ...
  draw(({ background, fill, line, point, rect, restore, save, stroke, strokeWeight }) => {
    ...
    for (const { position, direction } of lines) {
      ...
      const endX = direction === Direction.Right
        ? startX + cellSize
        : startX;

      const endY = direction === Direction.Right
        ? startY
        : startY + cellSize;
